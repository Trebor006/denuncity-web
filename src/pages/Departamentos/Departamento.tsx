...
const Departamento = () => {
    ...
    useEffect(() => {
        const cargarDepartamento = async () => {
            if (id) {
                try {
                    const response = await axios.get(`https://denuncity-backend-app-in7v2.ondigitalocean.app/departamentos/buscar?id=${id}`);
    ...
    const handleGuardarClick = async () => {
        ...
        try {
            ...
            if (isEditing && id) {
                const response = await axios.put(`https://denuncity-backend-app-in7v2.ondigitalocean.app/departamentos/${id}`, departamentoData);
                toast.success('Área de trabajo actualizada exitosamente');
                console.log('Respuesta del servidor:', response.data);
            } else {
                const response = await axios.post('https://denuncity-backend-app-in7v2.ondigitalocean.app/departamentos/registrar', departamentoData);
